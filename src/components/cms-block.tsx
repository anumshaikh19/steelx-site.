...
export function CmsBlock({ blockId, label, children, className = "" }: { blockId: string; label?: string; children: ReactNode; className?: string }) {
  ...
  useEffect(() => {
    ...
    const textTargets = Array.from(root.querySelectorAll<HTMLElement>(TEXT_SELECTOR)).filter(el => Array.from(el.children).length === 0);
    ...
    textTargets.forEach((el, index) => {
      ...
      el.addEventListener("blur", () => writeOverride(key, el.innerText));
    });

    imageTargets.forEach((img, index) => {
      const key = assetKey(location, blockId, index);
      const value = overrides[key];
      if (value && typeof value === "object") { if (value.src) img.src = value.src; if (value.alt !== undefined) img.alt = value.alt; }
      if (!editMode) return;
      img.style.outline = "1px dashed rgba(201,169,110,.7)";
      img.style.outlineOffset = "4px";
      img.title = "CMS: click to edit image URL / alt text";
      img.addEventListener("click", (event) => {
        event.preventDefault(); event.stopPropagation();
        const src = window.prompt("Image URL", img.currentSrc || img.src || "");
        if (src === null) return;
        const alt = window.prompt("Image alt text", img.alt || "");
        writeOverride(key, { src, alt: alt ?? img.alt });
        img.src = src; if (alt !== null) img.alt = alt;
      });
    });

    linkTargets.forEach((link, index) => {
      if (!editMode) return;
      link.dataset.cmsLink = String(index);
      link.addEventListener("click", (event) => {
        event.preventDefault(); event.stopPropagation();
        const href = window.prompt("Link URL", link.getAttribute("href") || "");
        if (href !== null) { link.setAttribute("href", href); writeOverride(`${location}::${blockId}::link-${index}`, href); }
      });
    });
    return () => {
      textTargets.forEach(el => { el.contentEditable = "false"; el.style.outline = ""; el.style.outlineOffset = ""; });
      imageTargets.forEach(img => { img.style.outline = ""; img.style.outlineOffset = ""; img.title = ""; });
    };
...
export function CmsVisualEditorHint() {
  ...
  return <div className="fixed right-5 top-24 z-[9999] rounded-2xl border border-[#c9a96e]/40 bg-[#171715] p-4 text-white shadow-2xl" style={{ maxWidth: 300 }}>
    <p className="text-[9px] uppercase tracking-[.2em] text-[#c9a96e]">Visual CMS · {location}</p>
    <p className="mt-2 text-xs leading-5 text-white/65">Click text to edit. Click an image to change its URL and alt text. Click a link to change its destination. Each section remains independently styled.</p>
