...
import { getPageSeo, savePageSeo, type PageSeo } from "@/lib/page-seo";

const TEXT_SELECTOR = "h1,h2,h3,h4,h5,h6,p,li,figcaption,blockquote,button";

function textKey(path: string, blockId: string, index: number) {
  return `${path}::${blockId}::${index}`;
}

function readTextOverrides(): Record<string, string> {
  try { return JSON.parse(localStorage.getItem("steelx-cms-text-v1") || "{}"); } catch { return {}; }
}

function writeTextOverride(key: string, value: string) {
  const all = readTextOverrides();
  all[key] = value;
  localStorage.setItem("steelx-cms-text-v1", JSON.stringify(all));
}
...
export function CmsBlock({ blockId, label, children, className = "" }: { blockId: string; label?: string; children: ReactNode; className?: string }) {
  ...
  useEffect(() => {
    ...
    setEditing(editMode);
    const targets = Array.from(root.querySelectorAll<HTMLElement>(TEXT_SELECTOR)).filter((el) => {
      if (el.closest("[data-cms-toolbar]")) return false;
  ...
  const design: PageSeo["design"] = {
    ...seo.design,
    ...(block.design || {}),
  };
  const cssVars = {
    "--cms-accent": design.accent,
    "--cms-bg": design.background,
    "--cms-heading": design.headingColor,
    "--cms-body": design.bodyColor,
  ...
  return <div ref={ref} data-cms-block={blockId} data-cms-label={label || blockId} className={`relative cms-block ${className}`} style={{ ...cssVars, background: design.background, ...(block.css ? { } : {}) }}>
    {block.css ? <style>{`[data-cms-block="${blockId}"]{${block.css}}`}</style> : null}
...
export function CmsVisualEditorHint() {
  ...
  const active = new URLSearchParams(window.location.search).get("cms-edit") === "1";
  ...
  return <div className="fixed right-5 top-24 z-[9999] rounded-2xl border border-[#c9a96e]/40 bg-[#171715] p-4 text-white shadow-2xl" style={{ maxWidth: 280 }}>
    ...
    <p className="mt-2 text-xs leading-5 text-white/65">Every visible text element is editable. Changes save to this browser as you leave each field.</p>
