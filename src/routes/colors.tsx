...
import { ColorsExperiences } from "@/components/colors-experiences";
      ...
      <main>
        <section className="colors-hero relative min-h-[92svh] overflow-hidden">
          ...
          <div className="relative z-10 mx-auto flex min-h-[92svh] max-w-[1700px] flex-col justify-between px-5 pb-8 pt-28 sm:px-8 lg:px-12 lg:pt-36">
            <div className="flex items-start justify-between gap-8"><p className="colors-eyebrow">STEELX / COLOUR LIBRARY</p><div className="hidden text-right sm:block"><p className="colors-micro">MATERIAL SYSTEM 01</p><p className="colors-micro mt-2 text-white/40">SS 304 / SS 316</p></div></div>
            <div className="max-w-[1050px] pb-10 lg:pb-14"><p className={`colors-kicker ${revealed ? "is-visible" : ""}`}>THE ARCHITECTURAL PALETTE</p><h1 className={`colors-title ${revealed ? "is-visible" : ""}`}>PVD Coated<br /><em>Stainless Steel</em><br />Surfaces</h1><div className="mt-8 flex max-w-[700px] flex-col gap-7 sm:flex-row sm:items-end sm:justify-between lg:mt-10"><p className="max-w-xl text-sm leading-7 text-white/60 sm:text-base sm:leading-8">Color becomes architecture. A controlled palette of stainless steel surfaces, tuned through PVD for depth, permanence and a precise visual language.</p><div className="flex shrink-0 gap-3"><Link to="/contact" className="colors-button-primary">Start a project <ArrowUpRight className="h-4 w-4" /></Link><a href="#library" className="colors-button-secondary">Explore colors <ArrowDown className="h-4 w-4" /></a></div></div></div>
            <div className="flex items-end justify-between border-t border-white/10 pt-4"><div className="flex gap-5 text-[9px] uppercase tracking-[0.28em] text-white/35"><span>SS 304</span><span>SS 316</span><span>PVD</span><span>0.8–3.0 MM</span></div><a href="#library" className="hidden items-center gap-3 text-[9px] uppercase tracking-[0.28em] text-white/45 transition-colors hover:text-[#d7b77d] sm:flex">Scroll to material library <ArrowDown className="h-3.5 w-3.5" /></a></div>
          </div>
        </section>

        <section id="library" className="relative overflow-hidden border-t border-white/10 bg-[#171512] px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto max-w-[1500px]"><div className="grid gap-14 lg:grid-cols-[0.8fr_1.2fr] lg:gap-24"><div><p className="colors-eyebrow">02 / CHROMATIC SYSTEM</p><h2 className="mt-5 max-w-md font-display text-4xl leading-[0.98] text-[#eee9de] sm:text-6xl">Six tones.<br />Infinite spaces.</h2><p className="mt-7 max-w-md text-sm leading-7 text-white/45">A restrained spectrum designed to move between warm hospitality, quiet residential interiors and monumental architectural envelopes.</p></div><div className="grid grid-cols-2 border-l border-t border-white/10 sm:grid-cols-3">{colors.map((color, index) => <article key={color.code} className="colors-swatch group border-b border-r border-white/10 p-4 sm:p-5"><div className="relative aspect-[1.15] overflow-hidden" style={{ background: color.tone }}><div className="absolute inset-0 bg-[linear-gradient(115deg,transparent_28%,rgba(255,255,255,.32)_44%,transparent_57%)] opacity-20 transition-all duration-1000 group-hover:translate-x-full group-hover:opacity-60" /><span className="absolute left-3 top-3 text-[8px] uppercase tracking-[0.2em] text-white/60">0{index + 1}</span></div><div className="flex items-end justify-between gap-3 pt-4"><div><h3 className="font-display text-xl text-white/90">{color.name}</h3><p className="mt-1 text-[8px] uppercase tracking-[0.24em] text-white/35">{color.code}</p></div><MoveUpRight className="h-4 w-4 text-white/25 transition-transform duration-500 group-hover:-translate-y-1 group-hover:translate-x-1 group-hover:text-[#d7b77d]" /></div></article>)}</div></div></div></section>

        <ColorsExperiences />

        <section className="border-t border-white/10 bg-[#11100e] px-5 py-24 sm:px-8 lg:px-12 lg:py-32"><div className="mx-auto grid max-w-[1500px] gap-14 lg:grid-cols-[1fr_1fr] lg:items-end"><div><p className="colors-eyebrow">07 / MATERIAL INTELLIGENCE</p><h2 className="mt-5 max-w-2xl font-display text-4xl leading-[1.02] sm:text-6xl">Surface is not decoration.<br /><span className="text-[#c9a46b]">It is the finish of the architecture.</span></h2></div><div className="grid grid-cols-2 border-l border-t border-white/10 sm:grid-cols-4">{["SS 304", "SS 316", "PVD", "0.8–3.0 mm"].map((item, i) => <div key={item} className="border-b border-r border-white/10 p-5 sm:p-6"><p className="text-[8px] uppercase tracking-[0.25em] text-white/30">0{i + 1}</p><p className="mt-8 font-display text-lg text-white/85">{item}</p></div>)}</div></div></section>
        <section className="relative overflow-hidden border-t border-white/10 bg-[#d8c39a] px-5 py-24 text-[#151411] sm:px-8 lg:px-12 lg:py-32"><div className="absolute -right-20 -top-40 h-[500px] w-[500px] rounded-full bg-white/25 blur-3xl" /><div className="relative mx-auto flex max-w-[1500px] flex-col gap-10 lg:flex-row lg:items-end lg:justify-between"><div><p className="colors-eyebrow text-[#151411]/50">08 / THE NEXT SURFACE</p><h2 className="mt-5 max-w-4xl font-display text-5xl leading-[0.95] sm:text-7xl lg:text-8xl">Specify the color.<br />We engineer the surface.</h2></div><Link to="/contact" className="inline-flex w-fit items-center gap-3 rounded-full border border-[#151411]/40 bg-[#151411] px-7 py-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-[#eee7d9] transition-transform duration-500 hover:-translate-y-1">Discuss your project <ArrowUpRight className="h-4 w-4" /></Link></div></section>
