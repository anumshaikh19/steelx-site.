...
import { LuxuryProjectHero } from "@/components/luxury-project-hero";
...
function HomePage() {
  ...
  return (
    <PageShell overlayHeader>
      <LuxuryProjectHero projects={projects} />
          ...
          {capabilities.map((c) => (
            <article key={c.t} className="group" data-cursor="Explore">
              <div className="aspect-[4/5] overflow-hidden border border-border metal-sheen">
                <img src={c.img} alt={c.t} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1400ms] ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-105" />
              </div>
      ...
      <Section className="border-t border-border">
        ...
        <Reveal variant="scale" className="mt-12"><FeatureProjectCard project={feature} /></Reveal>
        <div className="mt-20 grid gap-x-10 gap-y-16 sm:grid-cols-2 lg:grid-cols-3">
          {selected.map((p, i) => <Reveal key={p.slug} variant="up" delay={(i % 3) * 110}><ProjectGridCard project={p} index={i} /></Reveal>)}
        </div>
        {secondFeature ? <Reveal variant="scale" className="mt-20"><FeatureProjectCard project={secondFeature} /></Reveal> : null}
        <div className="mt-20 grid gap-x-10 gap-y-16 sm:grid-cols-2 lg:grid-cols-3">
          {more.map((p, i) => <Reveal key={p.slug} variant="up" delay={(i % 3) * 110}><ProjectGridCard project={p} index={i + 4} /></Reveal>)}
        </div>
        <Reveal variant="up" className="mt-16">
          <Link to="/projects" data-cursor="View all →" className="inline-flex items-center gap-2 border border-border px-8 py-4 text-[0.72rem] uppercase tracking-[0.2em] text-foreground transition-colors hover:border-champagne hover:text-champagne">All projects <ArrowUpRight className="h-4 w-4" /></Link>
        </Reveal>
      </Section>

      <Section className="border-t border-border">
        <SectionHeading eyebrow="From sheet to surface" title="How a panel is made" />
        <div className="mt-12"><StickyStory steps={story} /></div>
      </Section>

      <Section className="border-t border-border">
        <div className="flex flex-wrap items-end justify-between gap-6">
          <SectionHeading eyebrow="Finishes" title="Nine tones, one control sample" />
          <Link to="/ss-decorative-mesh-pvd" data-cursor="Explore →" className="text-[0.66rem] uppercase tracking-[0.22em] text-champagne">See decorative mesh →</Link>
        </div>
        <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {finishes.map((f, i) => <Reveal key={f.id} variant="up" delay={(i % 5) * 80}><div className="aspect-square border border-border metal-grain transition-transform duration-700 hover:scale-[1.03]" style={{ backgroundImage: f.swatch }} role="img" aria-label={`${f.name} finish`} /><p className="mt-3 text-[0.6rem] uppercase tracking-[0.24em] text-muted-foreground">{f.name}</p></Reveal>)}
        </div>
      </Section>
      ...
      <Section>
        <div className="flex flex-wrap items-end justify-between gap-6"><SectionHeading eyebrow="Journal" title="Notes on metal" /><Link to="/journal" className="text-[0.66rem] uppercase tracking-[0.22em] text-champagne">All articles →</Link></div>
        <div className="mt-12 grid gap-x-10 gap-y-14 sm:grid-cols-2 lg:grid-cols-3">
          {posts.map((p, i) => <Reveal key={p.slug} variant="up" delay={(i % 3) * 100}><Link to="/journal/$slug" params={{ slug: p.slug }} data-cursor="Read →" className="group block"><div className="aspect-[4/3] overflow-hidden border border-border metal-sheen"><img src={p.hero} alt={p.heroAlt} loading="lazy" className="h-full w-full object-cover transition-transform duration-[1300ms] group-hover:scale-105" /></div><p className="mt-4 text-[0.58rem] uppercase tracking-[0.26em] text-champagne">{p.category} · {formatDate(p.date)}</p><h3 className="mt-3 font-display text-xl leading-snug text-foreground transition-colors group-hover:text-champagne">{p.title}</h3></Link></Reveal>)}
        </div>
      </Section>

      <Section className="border-t border-border text-center">
        <Reveal variant="up"><h2 className="font-display text-4xl leading-tight text-foreground sm:text-7xl">Let's build a surface.</h2></Reveal>
        <Reveal variant="up" delay={100}><p className="mx-auto mt-6 max-w-xl text-sm leading-relaxed text-muted-foreground">Send drawings, a finish reference, or a photograph of a surface you like. We reply with samples and a specification.</p></Reveal>
        <Reveal variant="up" delay={180}><Magnetic className="mt-10"><Link to="/contact" data-cursor="Start →" className="inline-flex items-center gap-2 bg-champagne-gradient px-9 py-4 text-[0.72rem] font-semibold uppercase tracking-[0.2em] text-metal-black">Start a project <ArrowUpRight className="h-4 w-4" /></Link></Magnetic></Reveal>
      </Section>
    </PageShell>
  );
}
